...
<style>
  /* Bullet list direction for RTL/LTR */
  html[dir="rtl"] ul,
  html[dir="rtl"] ol {
    direction: rtl;
    text-align: right;
...
<script>
  // Debug: Add error handling and log API responses
  async function translateText(text, sourceLang, targetLang) {
    try {
      const response = await fetch('https://api.deepseek.com/translate', {
        method: 'POST',
        headers: {
          'Authorization': 'Bearer sk-0a7304558ad84efca277d285ce0059d7',
  ...
  // Helper: get all visible text nodes in the body
  function getTextNodes(root) {
    const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT, {
      acceptNode: function(node) {
        // Only non-empty, visible text nodes
  ...
  async function translatePage(sourceLang, targetLang, direction) {
    document.documentElement.setAttribute('dir', direction);
    // Only translate elements with .translatable-text
    const elements = document.querySelectorAll('.translatable-text');
...
<script>
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateContent(lang) {
      if (lang === 'ar') {
        mainHeadline.textContent = content.ar.headline;
        mainParagraph.textContent = content.ar.paragraph;
        mainButton.textContent = content.ar.button;
      } else {
        mainHeadline.textContent = content.en.headline;
        mainParagraph.textContent = content.en.paragraph;
        mainButton.textContent = content.en.button;
      }
    }

    // Initial content load
    var lang = localStorage.getItem('templateCustomizer-landing-front--Lang') || 'ar';
    updateContent(lang);

    if (arBtn) {
      arBtn.onclick = async function(e) {
        e.preventDefault();
        await translatePage('en', 'ar', 'rtl');
      };
    }
    if (enBtn) {
      enBtn.onclick = async function(e) {
        e.preventDefault();
        await translatePage('ar', 'en', 'ltr');
      };
    }
  });
...
<div data-bs-spy="scroll" class="scrollspy-example">
      ...
      <div class="container position-relative z-2">
        <div class="row align-items-center flex-row-reverse">
          ...
          <div class="col-lg-6 text-center text-lg-start" style="margin-top: rem;">
            ...
            <div id="app">
              <hero-switcher></hero-switcher>
            </div>
            <!-- Main headline and paragraph for translation -->
            <h1 id="main-headline" class="translatable-text display-4 fw-bold mb-3" style="color:#22304a;"></h1>
            <p id="main-paragraph" class="translatable-text lead mb-3 fw-bold" style="color:#22304a;"></p>
            ...
            <section id="landingSubjectStatements"
              ...
              <div class="w-100 text-center position-relative" style="z-index:3;">
                <div style="margin-top: 2.5rem;">
                  <h2 class="big-welcome-headline mb-3 translatable-text" style="margin-top: 1rem; font-size:2rem;">{{
                    __('Welcome to the future of your business') }}</h2>
                </div>
                <h2 class="lead mb-3 fw-bold" style="color:#22304a;">{{ __('Start a journey with modern CSS systems')
                  }}</h2>
                <a href="#landingPricing" class="btn btn-lg btn-glow px-5 py-3 mt-1"
                  style="font-size:1.25rem; border-radius:2rem; font-weight:bold; letter-spacing:1px;"
                  class="translatable-text">{{ __('Get Started')
                  }}</a>
            ...
            <div class="mt-5"
              style="max-width: 520px; margin-left: 0; margin-right: auto; margin-top: 2rem; padding-left: 2rem; padding-right: 1rem;">
              <div id="animated-statements"
                style="color:#22304a; line-height:2; font-size:3rem; font-weight:bold; text-align:center; margin-left:10rem;">
                <span class="lead translatable-text statement-line"
                  style="opacity:0; display:block; font-weight:bold;">{{ __('🚀 Take your business to the future')
                  }}</span>
                <span class="lead translatable-text statement-line"
                  style="opacity:0; display:block; font-weight:bold;">{{ __('💡All you need, in one platform') }}</span>
                <span class="lead translatable-text statement-line"
                  style="opacity:0; display:block; font-weight:bold;">{{ __('📊Control, analyze with modern systems')
                  }}</span>
                <span class="lead translatable-text statement-line"
                  style="opacity:0; display:block; font-weight:bold;">{{ __('✨Join and succeed together') }}</span>
                <span class="lead translatable-text statement-line"
                  style="opacity:0; display:block; font-weight:bold;">{{ __('💼Integrated business management')
                  }}</span>
              </div>
              <script>
                document.addEventListener('DOMContentLoaded', function () {
                  var lines = document.querySelectorAll('#animated-statements .statement-line');
                  lines.forEach(function(line, i) {
                    setTimeout(function() {
                      line.style.transition = 'opacity 0.7s';
              ...
              <style>
                #animated-statements .statement-line {
                  will-change: opacity;
                }
              </style>
              </p>
  ...
  <section id="landingFeatures" class="section-py landing-features">
    <div class="container">
      ...
      <h4 class="text-center mb-1">
        <span class="position-relative fw-extrabold z-1 translatable-text">{{ __('Everything you need') }}
        </span>
        <span class="translatable-text">{{ __('for modern Software & ERP UI') }}</span>
      </h4>
      <p class="text-center mb-12">{{ __('Our CSS toolkit empowers your software and ERP systems with professionally')
        }}</p>
      <div class="features-icon-wrapper row gx-0 gy-6 g-sm-12">
        <div class="col-lg-4 col-sm-6 features-icon-box d-flex flex-column align-items-center justify-content-center">
          <div class="icon-feature mb-3 d-flex align-items-center justify-content-center" style="height:80px;">
            <img src="{{asset('assets/img/front-pages/icons/laptop.png')}}" alt="Responsive Design" />
          </div>
          <h5 class="mb-2 text-center translatable-text">{{ __('Responsive Design') }}</h5>
          <p class="features-icon-description text-center translatable-text">{{ __('Works on any device size') }}</p>
        </div>
        <div class="col-lg-4 col-sm-6 features-icon-box d-flex flex-column align-items-center justify-content-center">
          <div class="icon-feature mb-3 d-flex align-items-center justify-content-center" style="height:80px;">
            <img src="{{asset('assets/img/front-pages/icons/rocket.png')}}" alt="Performance" />
          </div>
          <h5 class="mb-2 text-center translatable-text">{{ __('High Performance') }}</h5>
          <p class="features-icon-description text-center translatable-text">{{ __('Smooth, fast-loading styles') }}</p>
        </div>
        <div class="col-lg-4 col-sm-6 features-icon-box d-flex flex-column align-items-center justify-content-center">
          <div class="icon-feature mb-3 d-flex align-items-center justify-content-center" style="height:80px;">
            <img src="{{asset('assets/img/front-pages/icons/paper.png')}}" alt="Customizable" />
          </div>
          <h5 class="mb-2 text-center translatable-text">{{ __('Customizable Themes') }}</h5>
          <p class="features-icon-description text-center translatable-text">{{ __('Easily adapt styles') }}</p>
        </div>
        <div class="col-lg-4 col-sm-6 features-icon-box d-flex flex-column align-items-center justify-content-center">
          <div class="icon-feature mb-3 d-flex align-items-center justify-content-center" style="height:80px;">
            <img src="{{asset('assets/img/front-pages/icons/check.png')}}" alt="Utility Classes" />
          </div>
          <h5 class="mb-2 text-center translatable-text">{{ __('Utility Classes') }}</h5>
          <p class="features-icon-description text-center translatable-text">{{ __('Speed up with powerful features')}}
          </p>
        </div>
        <div class="col-lg-4 col-sm-6 features-icon-box d-flex flex-column align-items-center justify-content-center">
          <div class="icon-feature mb-3 d-flex align-items-center justify-content-center" style="height:80px;">
          ...
          <h5 class="mb-2 text-center translatable-text">{{ __('Accessibility') }}</h5>
          <p class="features-icon-description text-center translatable-text">{{ __('Built-in accessible interfaces') }}
          </p>
        </div>
        <div class="col-lg-4 col-sm-6 features-icon-box d-flex flex-column align-items-center justify-content-center">
          <div class="icon-feature mb-3 d-flex align-items-center justify-content-center" style="height:80px;">
          ...
          <h5 class="mb-2 text-center translatable-text">{{ __('Comprehensive Docs') }}</h5>
          <p class="features-icon-description text-center translatable-text">{{ __('Easy docs, rapid onboarding') }}
          </p>
        </div>
      </div>
    </div>
        ...
        <div class="col-xl-6 col-lg-6 mb-4">
          ...
          <div class="card" style="background:#FFF7B2;">
            <div class="card-header border-0" style="background:#FFF7B2;">
              <h2 class="fw-bold mb-0" style="color:#111;font-size:2.3rem;" id="focus-erp-title"
                class="translatable-text">
                {{ __('Focus ERP Programs') }}
              </h2>
              ...
              [
              'url' => '/product-details',
              'text_ar' => '• برنامج المبيعات والعملاء',
              'text_en' => ' .Sales and customer program'
              ],
              [
              'url' => '/product-details',
              'text_ar' => '• برنامج الموردين والمشتريات',
              'text_en' => ' .Supplier and procurement program'
              ],
              [
              'url' => '/product-details',
              'text_ar' => '• تكامل مباشر مع النظام الضريبي',
              'text_en' => 'Direct integration with the tax system'
              ],
              [
              'url' => '/product-details',
              'text_ar' => '• برنامج الحسابات العامة',
              'text_en' => ' .General accounts Program'
              ],
              [
              'url' => '/product-details',
              'text_ar' => '• برنامج الرقابة على الإنتاج والتصنيع',
              'text_en' => ' .Production and manufacturing control program'
              ],
              [
              'url' => '/product-details',
              'text_ar' => '• برنامج الموارد البشرية والرواتب',
              'text_en' => ' .Human resources and payroll program'
              ],
              [
              'url' => '/product-details',
              'text_ar' => '• برنامج الاعتمادات المستندية',
              'text_en' => ' .Documentary Credits Program'
              ],
              [
              'url' => '/product-details',
              'text_ar' => '• برنامج الصيانة',
              'text_en' => ' .Maintenance Program'
              ],
              [
              'url' => '/product-details',
              'text_ar' => '• الميزات العامة للبرنامج',
              'text_en' => ' .General features of the program'
              ],
              [
              'url' => '/product-details',
              'text_ar' => '• نقاط التواصل',
              'text_en' => ' .Points of Contact'
              ],
              [
              'url' => '/product-details',
              'text_ar' => 'والمزيد',
              ...
              <ul class="list-unstyled"
                style="color:#111;font-size:1.1rem; direction: {{ (app()->getLocale() == 'ar' || $lang == 'ar') ? 'rtl' : 'ltr' }}; text-align: {{ (app()->getLocale() == 'ar' || $lang == 'ar') ? 'right' : 'left' }};">
                @foreach($focusFeatures as $feature)
                <li class="translatable-text"
                  style="text-align: {{ (app()->getLocale() == 'ar' || $lang == 'ar') ? 'right' : 'left' }};">
                  {{ (app()->getLocale() == 'ar' || $lang == 'ar') ? $feature['text_ar'] : $feature['text_en'] }}
                </li>
                @endforeach
              </ul>
        ...
        <div class="col-xl-6 col-lg-6 mb-4">
          ...
          <div class="card" style="background:#d8d8e7;">
            <div class="card-header border-0" style="background:#d8d8e7;">
              <h2 class="fw-bold mb-0 translatable-text" style="color:#111;font-size:2.3rem;" id="erplite-title">
                ERPLite
              </h2>
              <p class="fw-bold mt-2 mb-2 translatable-text" style="color:#222;">{{ __('Support Special Performance') }}
              </p>
              <hr style="border-top:2px solid #111;width:70%;margin:auto;">
              <p class="fw-bold mt-2 mb-2" style="color:#222;">{{ __('Special Features') }}</p>
              @php
              ...
              [
              'url' => '/product-details',
              'text_ar' => '• الإدارة الشاملة للمبيعات والمخزون والمشتريات والبنوك',
              'text_en' => ' Comprehensive management of sales, inventory, purchasing, and banking.'
              ],
              [
              'url' => '/product-details',
              'text_ar' => '• التكامل الكامل بين الأقسام (المبيعات، المشتريات، المحاسبة)',
              'text_en' => ' Full integration between departments (sales, purchasing, and accounting).'
              ],
              [
              'url' => '/product-details',
              'text_ar' => '• تنبيهات ذكية للحدود الائتمانية ومواعيد الاستحقاق',
              'text_en' => ' Smart alerts for credit limits and due dates.'
              ],
              [
              'url' => '/product-details',
              'text_ar' => '• تحديد دقيق لعمولات المندوبين',
              'text_en' => ' Accurately determine representative commissions.'
              ],
              [
              'url' => '/product-details',
              'text_ar' => '• تقارير ذكية مفصلة',
              'text_en' => ' Detailed Smart Reports.'
              ],
              [
              'url' => '/product-details',
              'text_ar' => '• الإدارة الذكية لموارد الشركة والمخزون',
              'text_en' => ' Smart management of company resources and inventory.'
              ],
              [
              'url' => '/product-details',
              'text_ar' => '• التحكم الكامل في الحسابات والمعاملات',
              'text_en' => ' Full control of accounts and transactions.'
              ],
              [
              'url' => '/product-details',
              'text_ar' => '• تقليل الهدر وزيادة كفاءة العمليات اليومية',
              'text_en' => ' Reduces waste and increases the efficiency of daily operations.'
              ],
              [
              'url' => '/product-details',
              'text_ar' => '• متوافق مع قارئات الباركود والأجهزة المختلفة',
              'text_en' => ' Compatible with barcode scanners and various devices.'
              ],
              [
              'url' => '/product-details',
              'text_ar' => 'والمزيد',
              ...
              <ul class="list-unstyled"
                style="color:#111;font-size:1.1rem; direction: {{ (app()->getLocale() == 'ar' || $lang == 'ar') ? 'rtl' : 'ltr' }}; text-align: {{ (app()->getLocale() == 'ar' || $lang == 'ar') ? 'right' : 'left' }};">
                @foreach($erpliteFeatures as $feature)
                <li class="translatable-text"
                  style="text-align: {{ (app()->getLocale() == 'ar' || $lang == 'ar') ? 'right' : 'left' }};">
                  {{ (app()->getLocale() == 'ar' || $lang == 'ar') ? $feature['text_ar'] : $feature['text_en'] }}
                </li>
        ...
        <div class="col-xl-6 col-lg-6 mb-4">
          ...
          <div class="card" style="background:#c6e7ee;">
            <div class="card-header border-0" style="background:#c6e7ee;">
              <h2 class="fw-bold mb-0" style="color:#111;font-size:2.3rem;" id="hr-title" class="translatable-text">
                {{ __('HR') }}
              </h2>
              ...
              [
              'url' => '/product-details',
              'text_ar' => '• حساب تلقائي للرواتب والضرائب والتأمينات',
              'text_en' => '• Automatic payroll, tax, and insurance calculations.'
              ],
              [
              'url' => '/product-details',
              'text_ar' => '• دعم متغيرات الرواتب (الحوافز، الخصومات، القروض)',
              'text_en' => '• Support for payroll variables (incentives, deductions, loans).'
              ],
              [
              'url' => '/product-details',
              'text_ar' => '• تكامل مباشر مع أجهزة البصمة',
              'text_en' => '• Direct integration with fingerprint devices.'
              ],
              [
              'url' => '/product-details',
              'text_ar' => '• إدارة شاملة للغياب والتأخير والإجازات',
              'text_en' => '• Comprehensive management of absence, tardiness, and vacations.'
              ],
              [
              'url' => '/product-details',
              'text_ar' => '• دعم سياسات مرنة لكل موظف',
              'text_en' => '• Support for flexible policies tailored to each employee.'
              ],
              [
              'url' => '/product-details',
              'text_ar' => '• توفير الوقت والجهد في إدارة شؤون الموظفين',
              'text_en' => '• Saving time and effort in managing personnel affairs.'
              ],
              [
              'url' => '/productdetails',
              'text_ar' => '• دقة عالية في حساب الرواتب والتأمينات',
              'text_en' => '• High accuracy in calculating payroll and insurance.'
              ],
              [
              'url' => '/product-details',
              'text_ar' => '• مراقبة وإدارة أداء الموظفين بفعالية',
              'text_en' => '• Effectively monitoring and managing employee performance.'
              ],
              [
              'url' => '/product-details',
              'text_ar' => '• مرونة في توزيع وتنظيم الورديات',
              'text_en' => '• Flexibility in structuring and distributing shifts.'
              ],
              [
              'url' => '/product-details',
              'text_ar' => 'والمزيد',
        ...
        <div class="col-xl-6 col-lg-6 mb-4">
          ...
          <div class="card" style="background:#e0f7fa;">
            <div class="card-header border-0" style="background:#e0f7fa;">
              <h2 class="fw-bold mb-0" style="color:#111;font-size:2.3rem;" id="einvoice-title"
                class="translatable-text">
                {{ __('E-Invoice') }}
              </h2>
              ...
              [
              'url' => '/product-details',
              'text_ar' => '• متوافق مع معايير مصلحة الضرائب المصرية',
              'text_en' => '• Compliant with Egyptian Tax Authority standards.'
              ],
              [
              'url' => '/product-details',
              'text_ar' => '• يدعم ترميز EGS/GS1 الموحد',
              'text_en' => '• Supports EGS/GS1 unified coding.'
              ],
              [
              'url' => '/product-details',
              'text_ar' => '• تكامل مباشر مع النظام الضريبي',
              ...
              [
              'url' => '/product-details',
              'text_ar' => '• توقيع إلكتروني موثق',
              'text_en' => '• Authenticated electronic signature.'
              ],
              [
              'url' => '/product-details',
              'text_ar' => '• إمكانية استيراد البيانات من Excel',
              'text_en' => '• Ability to import data from Excel.'
              ],
              [
              'url' => '/product-details',
              'text_ar' => '• سهولة إرسال واعتماد الفواتير من الجهات الضريبية',
              'text_en' => '• Ease of sending and approving invoices from tax authorities.'
              ],
              [
              'url' => '/product-details',
              'text_ar' => '• تقليل أخطاء المحاسبة',
              'text_en' => '• Reduced accounting errors.'
              ],
              [
              'url' => '/product-details',
              'text_ar' => '• تسريع العمليات الضريبية وتوفير الجهد',
              'text_en' => '• Speed up tax processes and saved effort.'
              ],
              [
              'url' => '/product-details',
              'text_ar' => '• دعم فني وتدريب شامل',
              'text_en' => '• Comprehensive technical support and training.'
              ],
              [
              'url' => '/product-details',
              'text_ar' => 'والمزيد',
              ...
              <ul class="list-unstyled"
                style="color:#111;font-size:1.1rem; direction: {{ (app()->getLocale() == 'ar' || $lang == 'ar') ? 'rtl' : 'ltr' }}; text-align: {{ (app()->getLocale() == 'ar' || $lang == 'ar') ? 'right' : 'left' }};">
                @foreach($einvoiceFeatures as $feature)
                <li class="translatable-text"
                  style="text-align: {{ (app()->getLocale() == 'ar' || $lang == 'ar') ? 'right' : 'left' }};">
                  {{ (app()->getLocale() == 'ar' || $lang == 'ar') ? $feature['text_ar'] : $feature['text_en'] }}
                </li>
  ...
  <section id="landingGraphs" class="section-py landing-graphs bg-body"
    style="background: linear-gradient(120deg, #010508 60%, #010207 100%);">
    <div class="container">
      ...
      <div class="text-center mb-4">
        <span class="badge bg-label-primary dashboard-badge">{{ __('Interactive dashboard') }}</span>
      </div>
      <h4 class="text-center mb-1 dashboard-heading">
        <span class="position-relative fw-extrabold z-1">رؤية بيانية متقدمة لأعمالك</span>
      </h4>
      <p class="text-center mb-12 dashboard-desc">{{ __('Track performance indicators, sales, HR, and more with
        interactive and attractive charts.') }}</p>
  ...
  <section id="landingFAQ" class="section-py bg-body landing-faq">
    <div class="container">
      ...
      <h4 class="text-center mb-1">
        <span class="position-relative fw-extrabold z-1">{{ __('Frequently asked questions') }}
        </span>
      </h4>
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    if (isMobile) {
        ...
        alert('You are using a computer.');
    }
});
</script>